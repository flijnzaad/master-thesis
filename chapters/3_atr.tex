...
\todo[inline]{try to cut down on details as much as possible}
...
\todo[inline]{the below part: put it before the part on modules?}
...
\todo{add a bit more: on their opinion that NHA don't reason}
...
Moreover, the authors discuss findings on group reasoning tasks, in which participants are first tasked with solving problems individually, then in a small group, and lastly individually again. On for example the Wason selection task, participants' performance improved dramatically in a group setting \citep{Moshman98} \todo{Doing what? One example would be illustrative}.
