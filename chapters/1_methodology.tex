...
Ultimate causes on the other hand are the higher-level historical and evolutionary explanation of those traits \citep{Mayr61}\largeTodo{Add an example to illustrate}. In other words, these two different causes answer two different explanatory questions \smallTodo{Do causes answer questions? I know what you mean but it doesn't sound right}: the proximate cause is the answer to the \emph{how}-question, whereas the ultimate cause answers the \emph{why}-question. According to Mayr \smallTodo{Add reference to Mayr}, who pioneered the distinction \citep{Laland13}, one needs to answer both of these questions in order to obtain a complete understanding of the phenomenon at hand \smallTodo{Trait? Behavior? Ability? This is vague}.
...
\smallTodo{Add references: where did you read this?}
...
\smallTodo{The actual ultimate origin of it is Aristotle; make mention of this}
...
\largeTodo{What about reciprocal causation and niche construction? Or cultural learning?}
...
\largeTodo{This "direct result of natural selection" is very vague; elaborate on this}
...
\smallTodo{Move this bit upwards}
...
\largeTodo{Add a section on culture to this chapter (where?): add things from \citet{Heyes18}, and maybe some things from \citet{Laland13} and \citet{LB02}.}
...
\largeTodo{Rewrite this: first reformulate Tinbergen's questions (in enumeration with (i) would be appropriate), then say the first one is out of scope. Also note that Tinbergen's paper was seminal, here or at first mention}
