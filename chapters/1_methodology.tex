...
Secondly, in standard evolutionary theory the sorting process amounts to natural selection and genetic drift. Natural selection \todo{Confusion of selection vs. sorting here: resolve}
...
Firstly, variation in culture is introduced by error and by innovation. \todo{Add example}
Secondly, sorting of behaviors and habits in culture can happen through two different routes. A behavior can be sorted (selected for) because of some property inherent to the behavior that makes it "more noticeable, learnable, or memorable than others" \citep[p.~34]{Heyes18} \todo{Add example} and thus more likely to be copied. Also, a behavior or habit may be sorted in a more 'classic' evolutionary way: a habit may be selected for because it improves the fitness of the individual, such that individuals with that habit are more likely to survive and reproduce than individuals with an alternative habit \todo{Add example}.
Thirdly, culture may be retained through cultural inheritance, that is, through mechanisms of social learning \todo{Add example}.
...
\todo[inline]{To do: Discuss how costs and benefits feature in evolutionary causation (or evolutionary theory?), because Chapter 4 talks a lot about costs and benefits.}
